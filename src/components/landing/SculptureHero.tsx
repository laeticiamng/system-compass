 ...
 * The product (Compass) is presented as a sculpture: layered, rotating,
 * internals revealed through scroll & interaction — no gimmicks.
 */
...
export function SculptureHero() {
  ...
  // Parchment-like reveal: sculpture rotates, internal rings expose materials
  ...
  // Cursor parallax — internal materials respond to interaction
  ...
  return (
    ...
    >
      ...
      {/* THE SCULPTURE — concentric rings as a layered instrument */}
      <motion.div
        style={{ rotate: sculptureRotate, scale: sculptureScale, y: sculptureY, rotateX: tiltX, rotateY: tiltY }}
      ...
      >
        ...
        <motion.div
          ...
          className="absolute inset-[8%] rounded-full border border-primary/15 backdrop-blur-[1px]"
          style={{ background: 'radial-gradient(circle at 30% 20%, hsl(var(--primary)/0.10), transparent 60%)' }}
        />
        {/* Inner ring — exposed mechanism */}
        <motion.div
          style={{ opacity: innerOpacity }}
          className="absolute inset-[18%] rounded-full border border-amber-400/20"
        >
          {/* Tick marks — like a compass dial */}
          {Array.from({ length: 36 }).map((_, i) => (
            <div
              key={i}
              className="absolute left-1/2 top-0 -translate-x-1/2 w-px bg-foreground/20"
              style={{
                height: i % 9 === 0 ? 18 : 8,
        ...
        {/* Core */}
        <div className="absolute inset-[36%] rounded-full"
          style={{
            background:
