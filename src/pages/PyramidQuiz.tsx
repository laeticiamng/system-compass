...
type GameMode = 'online' | 'board' | null;
...
export default function PyramidQuiz() {
  ...
  const [scores, setScores] = useState<Record<PyramidType, number>>({
    PROBLEM_RENT: 0,
    STABILITY_REDIS: 0,
    COMPETENCE_TRUST: 0,
    GROWTH_RISK: 0,
  ...
  const [boardScores, setBoardScores] = useState<Record<PyramidType, number>>({
    PROBLEM_RENT: 0,
    STABILITY_REDIS: 0,
    COMPETENCE_TRUST: 0,
    GROWTH_RISK: 0,
  ...
  const rollDice = () => {
    ...
    const rollInterval = setInterval(() => {
      ...
      if (rollCount >= 10) {
        ...
        const newPosition = Math.min(playerPosition + finalValue, 24);
        setPlayerPosition(newPosition);
        
        handleBoardSquare(newPosition);
      }
    }, 100);
  };
  ...
  const handleBoardSquare = (position: number) => {
    ...
    if (square.type === 'pyramid' && square.pyramid) {
      ...
      Object.keys(boardScores).forEach(type => {
        boardScores[type as PyramidType] += 1;
      });
          ...
          <div className="grid md:grid-cols-2 gap-8">
            ...
            >
              <div className="flex items-center gap-4 mb-6">
                ...
                <h2 className="font-display text-2xl font-semibold">{t('pyramidQuiz.modes.online.title')}</h2>
              </div>
              <p className="text-muted-foreground mb-6">{t('pyramidQuiz.modes.online.description')}</p>
            ...
            >
              <div className="flex items-center gap-4 mb-6">
                ...
                <h2 className="font-display text-2xl font-semibold">{t('pyramidQuiz.modes.board.title')}</h2>
              </div>
              <p className="text-muted-foreground mb-6">{t('pyramidQuiz.modes.board.description')}</p>
              <div className="flex items-center gap-2 text-primary font-medium">
      ...
      return (
        <div className="min-h-screen pt-24 pb-16">
          <div className="container mx-auto px-4 max-w-2xl">
            ...
            <div className={cn(
              "glass-card rounded-2xl p-8 mb-8 border-2",
                  ...
                  .map(([type, score]) => (
                    <div key={type} className="flex items-center gap-4">
                      ...
                      <div className="flex-1 h-3 bg-secondary rounded-full overflow-hidden">
                        <div 
                          className={cn("h-full rounded-full transition-all", PYRAMID_COLORS[type as PyramidType].split(' ')[0])}
        ...
        <div className="container mx-auto px-4 max-w-6xl">
          ...
          <div className="grid grid-cols-5 md:grid-cols-7 gap-2 mb-8">
            {BOARD_SQUARES.map((square, index) => {
              ...
              let squareClass = "aspect-square rounded-lg border-2 flex flex-col items-center justify-center text-xs p-1 relative transition-all";
          ...
          <div className="flex flex-col items-center gap-6">
            <button
              ...
              className={cn(
                "p-8 rounded-2xl bg-primary/10 hover:bg-primary/20 transition-all",
                isRolling && "animate-bounce"
              )}
            >
              <DiceIcon className="w-16 h-16 text-primary" />
            </button>
            
            <p className="text-muted-foreground">{t('pyramidQuiz.board.rollDice')}</p>
