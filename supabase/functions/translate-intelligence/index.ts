...
serve(async (req) => {
  ...
  try {
    const { countryId, targetLang, intelligenceData } = await req.json();

    if (!countryId || !targetLang || !intelligenceData) {
      return new Response(
    ...
    if (targetLang === "en") {
      return new Response(
        JSON.stringify({ translatedData: intelligenceData, cached: false }),
    ...
    const systemPrompt = `You are a professional translator specializing in socio-economic and geopolitical analysis. 
...
- Maintain the analytical, professional tone
    ...
    const response = await fetch("https://ai.gateway.lovable.dev/v1/chat/completions", {
      ...
      body: JSON.stringify({
        ...
        messages: [
          ...
          { role: "user", content: JSON.stringify(intelligenceData) },
        ],
      }),
    });
