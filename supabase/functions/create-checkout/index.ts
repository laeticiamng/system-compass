...
serve(async (req) => {
  ...
  // Rate limit: 10 checkouts / 5 min / IP — protects against checkout spam
  const rl = checkRateLimit(getRateLimitKey(req, 'create-checkout'), { maxRequests: 10, windowSeconds: 300 });
  ...
  try {
    ...
    if (authError || !data.user?.email) {
      return new Response(
        JSON.stringify({ error: "User not authenticated or email not available", code: "AUTH_FAILED" }),
        { status: 401, headers: { ...corsHeaders, "Content-Type": "application/json" } }
      );
    }
